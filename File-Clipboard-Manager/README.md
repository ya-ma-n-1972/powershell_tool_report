...
> **✨ 新機能追加（2025-11-03）**: v1.1.0をリリースしました。クリップボード内のテキストやファイルを直接フェンスで囲む機能を追加しました。詳細は[バージョン履歴](#バージョン履歴)をご確認ください。
...
3. **クリップボード内容へのフェンス追加** ⭐ NEW
   - クリップボードにコピー済みのテキストやファイルを直接フェンスで囲む
   - 設定ウィンドウを開かずに素早く処理可能
   - 短いコードスニペットの整形に便利
...
#### 4. クリップボード内容にフェンス追加（新機能）

1. テキストまたはファイルをコピー（Ctrl+C）
2. メインウィンドウで**「クリップボードにフェンス追加」**ボタンをクリック
3. フェンスで囲まれた状態でクリップボードに再登録される
4. テキストエディタやチャットに貼り付け（Ctrl+V）

> **対応形式**: クリップボードにコピーされたテキストまたはファイル（1ファイルのみ、最大30MB、テキスト形式のみ）
...
### 例4: 短いコードスニペットをフェンスで囲む（新機能）

1. Webサイトやエディタから短いコードをコピー（Ctrl+C）
2. メインウィンドウで**「クリップボードにフェンス追加」**をクリック
3. Claude AIのチャット画面に貼り付け（Ctrl+V）
4. コードブロックとして正しく整形されて表示
...
### v1.1.0 (2025-11-03)
**新機能**
- **クリップボード内容へのフェンス追加機能**を追加
  - メインウィンドウに「クリップボードにフェンス追加」ボタンを追加
  - クリップボードにコピー済みのテキストまたはファイルを直接Markdownフェンス（` ``` `）で囲む
  - 設定ウィンドウを開かずに素早く処理可能

**対応形式**
- テキスト：クリップボードにコピーされた任意のテキスト（空白チェック付き）
- ファイル：エクスプローラーでコピーしたテキストファイル（1ファイルのみ、最大30MB）
  - 対応拡張子：.txt, .ps1, .psm1, .psd1, .cs, .vb, .js, .ts, .html, .htm, .xml, .json, .css, .md, .log, .csv, .ini, .config, .bat, .cmd, .py, .rb, .java, .c, .cpp, .h, .hpp, .php, .sh
...
```powershell
# テキストをコピーしてフェンス追加
1. どこかからコードをコピー（Ctrl+C）
2. 「クリップボードにフェンス追加」ボタンをクリック
3. Claudeに貼り付け（Ctrl+V）
```
